...
export class ACPBackend extends AIBackend {
  ...
  _saveSessionId(sessionId) {
    try {
      ...
      console.warn(`[ACP] Failed to save session ID: ${err.message}`);
    }
  }
  ...
  async _ensureSession() {
    ...
    if (!this._sessionId) {
      ...
      if (savedId) {
        try {
          console.log(`[ACP] Attempting to resume session: ${savedId}`);
          const result = await this._sendRequest('session/load', { sessionId: savedId });
          ...
          console.log(`[ACP] Resumed session: ${this._sessionId}`);
          return this._sessionId;
        } catch (err) {
          console.log(`[ACP] Could not resume session: ${err.message} — creating new`);
      ...
      // Cache available models from the session response
      // result.models is { currentModelId, availableModels: [{ modelId, name, description }] }
      if (result.models && result.models.availableModels) {
        this._models = {
          currentModelId: result.models.currentModelId || 'auto',
          availableModels: result.models.availableModels.map(m => ({
        ...
        console.log(`[ACP] Models: ${this._models.availableModels.map(m => m.id).join(', ')} (current: ${this._models.currentModelId})`);
      }
  ...
  async setModel(modelId) {
    ...
    console.log(`[ACP] Set model: ${modelId}`);
  ...
  async getCommands() {
    ...
    try {
      ...
      console.log(`[ACP] Commands: ${this._commands.map(c => c.name).join(', ')}`);
      return this._commands;
    } catch (err) {
      console.warn(`[ACP] Failed to fetch commands: ${err.message}`);
      return [];
    }
  }
  ...
  async getCommandOptions(input) {
    ...
    try {
      ...
      console.warn(`[ACP] Command options failed: ${err.message}`);
      return [];
    }
  }
  ...
  async executeCommand(command) {
    const sessionId = await this._ensureSession();
    console.log(`[ACP] Executing command: ${command}`);
  ...
  async *chatStream(messages, systemPrompt, imageAttachments) {
    try {
      const sessionId = await this._ensureSession();
      ...
      const handler = (msg) => {
        ...
        if (msg.method === 'session/update') {
          ...
          if (update?.sessionUpdate === 'agent_message_chunk') {
            ...
            // Tool invocation starting — surface to frontend
            const title = update.toolCall?.title || update.toolCall?.name || 'Tool call';
            const params = update.toolCall?.parameters;
            console.log(`[ACP] tool_call: ${title}`);
            enqueue({ type: 'tool_call', title, status: 'running', params });
          } else if (update?.sessionUpdate === 'tool_call_update') {
            // Progress update on a running tool call
            const title = update.toolCall?.title || update.toolCall?.name || 'Tool call';
            const progress = update.content?.text || update.progress || '';
            console.log(`[ACP] tool_call_update: ${title} — ${progress}`);
            enqueue({ type: 'tool_call_update', title, progress });
          } else if (update?.sessionUpdate === 'tool_result') {
            // Tool call completed
            const title = update.toolCall?.title || update.toolCall?.name || 'Tool call';
            console.log(`[ACP] tool_result: ${title}`);
            enqueue({ type: 'tool_result', title, status: 'done' });
        ...
        if (msg.method === '_kiro.dev/compaction/status') {
          ...
          console.log(`[ACP] Compaction status: ${status}`);
