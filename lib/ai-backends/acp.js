...
export class ACPBackend extends AIBackend {
  constructor(settings) {
    ...
    this._sessionCwd = null; // CWD used when creating the session (for resolving relative paths)
  ...
  /**
   * Extract a file path from an Obsidian URL or direct open command.
   * Returns the absolute path or null if not matched.
   */
  _extractOpenFilePath(title) {
    if (!title || typeof title !== 'string') return null;

    // Match: open obsidian://open?...file=X or obsidian://open?...path=X
    const obsidianMatch = title.match(/open\s+obsidian:\/\/open\?[^\s]*(?:file|path)=([^\s&]+)/i);
    if (obsidianMatch) {
      try {
        return decodeURIComponent(obsidianMatch[1]);
      } catch {
        return obsidianMatch[1];
      }
    }

    // Match: open <path>.md/txt/markdown — handles paths with spaces
    // Quoted: open "some path/file.md" or open 'some path/file.md'
    const quotedMatch = title.match(/open\s+["'](.+\.(?:md|markdown|txt))["']/i);
    if (quotedMatch) return quotedMatch[1];

    // Unquoted: open <everything up to the file extension>
    const directMatch = title.match(/open\s+(.+\.(?:md|markdown|txt))\s*$/i);
  ...
  /**
   * Resolve a file path to absolute using the session CWD.
   * Returns the resolved path, or the original if already absolute.
   */
  _resolveToAbsolute(filePath) {
    ...
    if (this._sessionCwd) return path.resolve(this._sessionCwd, filePath);
    return null;
  }

  /**
   * Extract file paths (ending in known extensions) from arbitrary text.
  ...
  _extractFilePathsFromText(text) {
    ...
    for (const line of text.split('\n')) {
      // Paths in backticks: `some/path/file.md`
      for (const m of line.matchAll(/`([^`]+)`/g)) {
        if (EXT.test(m[1])) paths.push(m[1].trim());
      }
      // Bare line that looks like a file path (contains / and ends with extension)
      const stripped = line.replace(/^[\s*->]*/, '').trim();
      if (EXT.test(stripped) && stripped.includes('/') && !stripped.startsWith('http')) {
        paths.push(stripped);
      }
    }
    return paths;
  ...
  _handleIncomingRequest(msg) {
    ...
    if (method === 'session/request_permission') {
      ...
      console.log(`[ACP ${ts()}] Permission request: ${title || 'unknown'} (edit mode: ${this.editModeEnabled})`);

      // Intercept Obsidian open commands — open in GhostWriter instead
      const openFilePath = this._extractOpenFilePath(title);
      if (openFilePath) {
        console.log(`[ACP ${ts()}] Intercepted file open: ${logPath(openFilePath)} — redirecting to GhostWriter`);
        // Emit open_file event via notification queue (will be picked up by chatStream)
        if (this._openFileCallback) {
          this._openFileCallback(openFilePath);
        }
        // Reject the shell command so Obsidian doesn't launch
  ...
  _sendRequest(method, params = {}, { timeout } = {}) {
    return new Promise((resolve, reject) => {
      ...
      // session/prompt is long-running (streams via notifications, response = turn end)
      // so it gets a much longer timeout than regular RPC calls
      const timeoutMs = timeout ?? (method === 'session/prompt' ? 600000 : 60000);
  ...
  async _initialize() {
    ...
    // Persistent listener for metadata notifications — keeps _contextUsage
    // fresh even between prompts (e.g., during compaction or background work).
    // The chatStream handler additionally enqueues events to the SSE stream.
    this._notifications.push((msg) => {
      if (msg.method === 'kiro.dev/metadata' || msg.method === '_kiro.dev/metadata') {
  ...
  async _doEnsureSession() {
    ...
    if (!this._sessionId) {
      ...
      if (savedId) {
        try {
          ...
          if (result.contextUsage) {
            this._contextUsage = this._normalizeContextUsage(result.contextUsage);
          }
      ...
      this._cacheModels(result);
      this._cacheModes(result);
      // Capture context usage from session creation if available
      if (result.contextUsage) {
        this._contextUsage = this._normalizeContextUsage(result.contextUsage);
      }
      if (this._models) {
      ...
      // Wait for MCP servers to initialize before returning — Kiro won't
      // process prompts until MCP servers are ready. Listen for
      // _kiro.dev/mcp/server_initialized notifications with a timeout.
  ...
  /**
   * Wait for MCP server initialization after session creation.
   * Kiro's custom modes (like milkis) spawn MCP servers asynchronously
   * and won't process prompts until they're ready. We listen for
   * _kiro.dev/mcp/server_initialized notifications and wait until they
   * stop arriving, using a quiet period to detect completion.
   */
  _waitForMcpReady() {
    return new Promise((resolve) => {
      ...
      const done = () => {
        ...
        if (count > 0) {
          console.log(`[ACP ${ts()}] MCP ready (${count} server(s) initialized)`);
        }
        resolve();
      };

      const handler = (msg) => {
        if (msg.method === '_kiro.dev/mcp/server_initialized') {
          count++;
          const name = msg.params?.serverName || 'unknown';
          console.log(`[ACP ${ts()}] MCP server ready: ${name}`);
          // Reset quiet timer — use 5s to catch slow servers (some
          // take ~4-5s to init while others finish in <1s)
          clearTimeout(quietTimer);
          quietTimer = setTimeout(done, 5000);
        }
      };

      this._notifications.push(handler);

      // Initial quiet timer — if no MCP notifications arrive within 3s,
      // assume no MCP servers to wait for (fast exit for modes without MCP)
      quietTimer = setTimeout(done, 3000);
      // Hard max — don't wait forever
      maxTimer = setTimeout(done, 30000);
    });
  }
  ...
  /**
   * Normalize context usage from various Kiro response shapes into { percentage }.
   * Kiro may send: { percentage }, { contextUsagePercentage }, or a raw number.
   */
  _normalizeContextUsage(raw) {
    if (!raw) return null;
    const pct = raw.percentage ?? raw.contextUsagePercentage ?? (typeof raw === 'number' ? raw : null);
    return pct != null ? { percentage: pct } : null;
  }
  ...
  async *chatStream(messages, systemPrompt, imageAttachments, documentResource, fileAttachments) {
    ...
    try {
      ...
      // --- Auto-open tracking ---
      // Detect if the user wants to open a file (vs. just searching/listing)
      const userMsg = messages.length > 0 ? messages[messages.length - 1]?.content || '' : '';
      const hasOpenIntent = /\b(open|show|display|load|view|pull\s+up|bring\s+up)\b/i.test(userMsg);
      let fileOpened = false;        // true once we've emitted an open_file event
      let accumulatedText = '';       // AI response text (for path extraction)
      const foundLocationPaths = [];  // file paths from tool_call_update locations

      // Set up open_file callback so permission handler can emit events into the stream
      this._openFileCallback = (filePath) => {
        const resolved = this._resolveToAbsolute(filePath);
        enqueue({ type: 'open_file', path: resolved || filePath });
        fileOpened = true;
      };
      ...
      handler = (msg) => {
        ...
        if (msg.method === 'session/update') {
          ...
          if (update?.sessionUpdate === 'agent_message_chunk') {
            ...
            // Check if this tool call is opening a file — emit open_file event
            const openPath = this._extractOpenFilePath(title);
            if (openPath) {
              const resolved = this._resolveToAbsolute(openPath);
              enqueue({ type: 'open_file', path: resolved || openPath });
              fileOpened = true;
            }
            ...
            // Capture file paths from tool locations (e.g., glob/search results)
            if (locations && locations.length > 0) {
              for (const loc of locations) {
                if (loc.path) foundLocationPaths.push(loc.path);
              }
            }
            ...
            if (update.contextUsage) {
              this._contextUsage = this._normalizeContextUsage(update.contextUsage) || this._contextUsage;
            }

            // Auto-open: if the user wanted to open a file and the AI found one
            // but never triggered an explicit open, do it now.
            if (hasOpenIntent && !fileOpened) {
              let targetPath = null;

              // Priority 1: file paths from tool locations (glob/search results)
              if (foundLocationPaths.length > 0) {
                targetPath = foundLocationPaths[0];
              }

              // Priority 2: file paths mentioned in the AI's text response
              if (!targetPath) {
                const textPaths = this._extractFilePathsFromText(accumulatedText);
                if (textPaths.length > 0) targetPath = textPaths[0];
              }

              if (targetPath) {
                const resolved = this._resolveToAbsolute(targetPath);
                if (resolved) {
                  console.log(`[ACP ${ts()}] Auto-open: detected file ${logPath(resolved)} from AI response`);
                  enqueue({ type: 'open_file', path: resolved });
                  fileOpened = true;
                }
              }
            }
        ...
        // Handle real-time context usage from Kiro metadata notifications
        if (msg.method === 'kiro.dev/metadata' || msg.method === '_kiro.dev/metadata') {
          const pct = msg.params?.contextUsagePercentage;
          if (pct != null) {
            this._contextUsage = { percentage: pct };
        ...
        // Handle compaction status from Kiro extensions
      ...
      // Stall detector — if Kiro fails internally it may never send a JSON-RPC
      // response (e.g. "Failed to send prompt" when MCP servers aren't ready).
      // If no activity arrives within 60s of the last activity, surface an error.
      ...
      const resetStallTimer = () => {
        ...
        if (!done) {
          stallTimer = setTimeout(() => {
            if (!done) {
              console.error(`[ACP ${ts()}] Stall detected — no activity from Kiro within ${STALL_TIMEOUT / 1000}s`);
              done = true;
              enqueue({ type: 'error', text: 'No response from Kiro — the prompt may have failed silently. Try sending again.' });
            }
          }, STALL_TIMEOUT);
        }
      };
      // Patch enqueue to reset stall timer on any activity
      ...
      this._sendRequest('session/prompt', { sessionId, prompt })
        .then((result) => {
          ...
          if (result?.contextUsage) {
            const normalized = this._normalizeContextUsage(result.contextUsage);
            if (normalized) {
              this._contextUsage = normalized;
              enqueue({ type: 'context_usage', ...normalized });
            }
          }
      ...
      // Always clear the stall timer and remove the notification handler,
      // even if the consumer exits early via break/return or an error is thrown.
      clearTimeout(stallTimer);
