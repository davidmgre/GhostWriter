...
export class ACPBackend extends AIBackend {
  ...
  _spawn() {
    ...
    try {
      ...
      console.log(`[ACP] Found ${cmd} at: ${logPath(which)}`);
    } catch {
  ...
  async _doEnsureSession() {
    ...
    if (!this._sessionId) {
      ...
      console.log(`[ACP] Creating session with cwd: ${logPath(cwd)}`);
  ...
  async *chatStream(messages, systemPrompt, imageAttachments, documentResource) {
    try {
      const sessionId = await this._ensureSession();
